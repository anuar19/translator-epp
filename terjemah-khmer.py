...
subscription_key = 'YOUR_AZURE_SUBSCRIPTION_KEY'  # Replace with your Azure Translator key
endpoint = 'https://api.cognitive.microsofttranslator.com/'  # Replace with your endpoint
location = 'global'  # Use your region if needed

def translate_to_khmer(text, retries=3):
    """
    Translate the given text from English to Khmer using Microsoft Translator API.
    """
    path = '/translate?api-version=3.0'
    params = '&from=en&to=km'
    ...
    while attempt < retries:
        try:
            print(f"Translating English text to Khmer: {text}")
            response = requests.post(constructed_url, headers=headers, json=body)
            response.raise_for_status()
            result = response.json()

            # Get the Khmer translation from the response
            khmer_translation = result[0]['translations'][0]['text']
            return khmer_translation
...
def update_json_file_for_khmer(input_file):
    # Load the JSON file
    try:
    ...
    def scan_and_translate(obj):
        ...
        if isinstance(obj, dict):
            if 'en-us' in obj:
                # English translation exists, translate to Khmer
                english_text = obj['en-us']
                khmer_text = translate_to_khmer(english_text)
    ...
    # Save the updated JSON file with '-khmer-updated' appended to the filename
    updated_file_name = os.path.splitext(input_file)[0] + '-km.json'
    try:
